...
## Operator F1/F4 panel ruling, 2026-09-06

Applies to the independent panel on c35bd34 (2026-09-06T05-02-54Z-FC-1-corrected-panel). These are explicit operator dispositions before separate seals/body corrections. Existing acceptance assertions remain; no known-red relaxation.

- **Role absence (Claude-1/Codex-5):** decodePredictiveFields intentionally maps absent/null role to empty. Empty is unknown, not authoritative disagreement; a valid cited role from another compatible reading supplies it regardless of citation order. A nonempty invalid role is Malformed after exclusions, never recovered. If ALL compatible readings lack role, the attempt cannot have a sample: retain the existing Unrecoverable disposition with explicit missing-role reason and PARTIAL aggregate. This is an explicit narrow clarification of Unrecoverable's prior measurement-only wording; it does not conflate missing role with AbsentStamp (which remains implementing-model only). Different valid roles still conflict. Never infer from authored model or task name.
- **Exact envelope association (Codex-4/Claude-4/Grok-2):** retain the exact classified Reading when grouping; equal Ref/Identity/CompletedAt do not make malformed/valid payloads interchangeable. Preserve every audit envelope and canonical permutation equality. Build a run/key index for misses rather than repeated map scans. No public JoinEvidence signature change: adding context violates the frozen seam. Build should check cancellation before entering join and keep a cancellation reason/PARTIAL diagnostic; cancellable inner joins are deferred API evolution.
- **EMPTY and source diagnostics (Claude-2/-3):** preserve ReadSources EMPTY through Build. Reduce/join/source failures may make PARTIAL but finalization cannot relabel an empty corpus COMPLETE. Every aggregate failure must have an actionable sorted/deduplicated reason; add source reason for early source failures that otherwise lack one. Individual successful source reports remain unchanged.
- **Structured eligibility (Codex-2/Claude-6):** invalid-payload refusal includes malformed required role/model/start/terminal/elapsed citations, invalid wall intervals, and contradictory evidence counters/audit. Recovered reading references and role citation must exist and be valid; journal citations must agree with the attempt; YAML terminal/elapsed must select the same cited reading. Unknown fields stay unknown (do not require unavailable optional measurements). Validate SummarizeWall before sampling; malformed interval input must not yield fabricated precise phase zeros. For valid COMPLETE artifacts, Conflicts/Ambiguous or malformed/unrecoverable dispositions are contradictions. Counts Recovered/Attempts/UniqueRows/lost identities/disposition counts/YAML-only terminal count must describe their arrays; legitimate losses such as absent stamps or no YAML are allowed, not blanket rejection of lost attempts. This is integrity validation, not reopening original journals or cryptographic certification.
  The synthetic recoveredArtifact helper currently supplies empty Readings/Examined/Dispositions and lacks Role evidence despite representing recovered eligible rows. Independent seals author is authorized to enrich ONLY this positive fixture into faithful synthetic joint evidence, preserving n, IDs, outcome, model, threshold, existing tests/assertions, and never calling the implementation under test to manufacture expected evidence. No blanket helper or production relaxation. Record before/after fixture details.
- **Attempt universe (Codex-3):** frozen contract requires each supplied set to belong to the discovered universe, not a bijection (incomplete reduction is allowed for diagnostics). Do not add an exactly-one-set-for-every-journal requirement. Repeated supplied set identities and duplicate normalized identities across regular/ambiguous/conflict categories are invalid, not overwrite/merge policy; reject consistently with ErrInvalidSelection for repeated set, ErrEvidenceConflict for duplicate category IDs. Multiple *different* conflict facts for one conflict-category identity may be retained; do not discard genuine conflicts. Missing selected set alone remains valid diagnostic input.
- **False-positive controls:** Grok-1 nil-manifest panic is disproved by explicitly nil-safe SourceManifest.ValidateComplete and existing F4-NOT-ELIGIBLE-PARTIAL nil test. Codex-6 interval mutation is prevented by canonicalAttempt deep-copying Wall.Intervals AND nested evidence before filtering (journal.go:1283 onward); add a repeated-join/unchanged-input control rather than claiming a fix. Sorting ambiguous Refs, however, must also not mutate caller-owned slices. Grok-4 zero Outcome claim is disproved by OutcomeDone=iota+1; zero is invalid. Codex-1's shipped CLI success claim is disproved by RunE's explicit missing --tasks refusal; add/retain this positive refusal control. A direct buildDispatchedReference helper call can bypass that guard; defensive typed refusal there after diagnostics is in scope, without changing CLI misuse into data-extraction success.
- **Documentation/diagnostics:** fix Codex-7 coverage gate wording; name uncomputed legacy Coverage counters and authoritative equivalents in Limits (Claude-5), and declare legacy Conflicts a compatibility field with Evidence.Conflicts authoritative (Claude-8). Label lost attempts accurately rather than 'unmatched siblings' (Grok-5). Remove redundant construction-time SilenceUsage so flag misuse can print usage, retaining RunE data-error suppression (Claude-7). Remove stale scaffold stub wording in FC-1-owned docs/comments.
