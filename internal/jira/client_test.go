...
func TestIsDoneStatus(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := isDoneStatus(tc.status, tc.doneStatuses); got != tc.want {
				t.Errorf("isDoneStatus(%q, %v) = %v, want %v", tc.status, tc.doneStatuses, got, tc.want)
			}
		})
	}
}
...
func TestExtractStateTransitionsAnchorsOnFirstDoneEquivalent(t *testing.T) {
	...
	t.Run("Awaiting Dev Deployment anchors when configured", func(t *testing.T) {
		...
		_, doneTime := c.extractStateTransitions(changelog, []string{"Done", "Awaiting Dev Deployment"})
		if doneTime == nil {
	...
	t.Run("Done anchors when only Done is configured", func(t *testing.T) {
		...
		_, doneTime := c.extractStateTransitions(changelog, []string{"Done"})
		if doneTime == nil {
			t.Fatal("expected doneTime to be set")
		}
	...
	t.Run("nil changelog returns nil times", func(t *testing.T) {
		inProgress, done := c.extractStateTransitions(nil, []string{"Done"})
	...
	t.Run("In Progress and In Development both anchor inProgressTime", func(t *testing.T) {
		...
		inProgress, _ := c.extractStateTransitions(changelog, []string{"Done"})
		if inProgress == nil {
