...
- No acceptance tests were authored (scaffold rule).
...
|---|---|---|
| `observation.go` | Baseline `Role/Outcome/Cell/Revision/Provenance/Observation` unchanged; NEW `AttemptID`, `Measured[T]`, `Phase`, `Interval`, `WallBreakdown`, `EvidenceSource`, `FieldEvidence`, `ObservationEvidence`; `Observation.AttemptID()`, `Observation.Wall`, `Observation.Evidence`. | FC-OBS-ADJ (disputed path) |
| `errors.go` | Baseline sentinels unchanged; NEW F1–F4 sentinels (table below). | scaffold (frozen) |
| `journal.go` | NEW frozen: producer constants, `JournalIdentity`, `EventRef`, `EventPayload`, `Event`, `JournalDiagnostics`, `Attempt`, `AttemptSet`, `AmbiguousAttempt`, `AttemptConflict`, holes `ParseEvents`, `ReduceAttempts`. MOVED from `extract.go`: `runTask`, `JournalFacts`, `JournalRow` (+methods), `journalEvent`, `journalPayload`, `journalSources`, `scanJournal`, both `observe` methods. | FC-JOURNAL |
| `sources.go` | NEW frozen: bound constants, `ReadBounds`, `SourceKind`, `SourceSpec`, `SourceState`, `SourceCounts`, `SourceReport`, `Selection`, `SourceManifest`, `ReadingRef`, `SourceReadings`, `gitEnvironment`, hole `ReadSources`. MOVED from `extract.go`: `readJournals`, `readJournal`, `readLiveSnapshots`, `gitSources`, `blobRef`, `readGitSnapshots`, `gitCatFileBatch`, `gitLines`, `readTargetTasks`, `defaultMaxHistoryCommits`. | FC-SOURCES |
...
| `extract.go` | Retains only YAML decoding: `yamlTask`, `yamlDocument`, `taskSnapshot` (+NEW `SourceID`, `Ref()`), `yamlSources`, `parseSnapshots`, `isTaskYAML`, `decodeTaskDocument`. | FC-1 (via `build.go` boundary) |
| `referenceclass.go` | Unchanged. FC-1 join/coverage boundary. | FC-1 |
...
- `EvidenceSource`: `EvidenceNone < EvidenceYAML < EvidenceJournal`.
  `FieldEvidence{Source, Event, Reading}`; `ObservationEvidence{Model, Terminal}`.
- `Attempt`: joint completed-attempt record (id, start ref, `Model
  Measured[string]` + evidence, cascades, outcome, terminal evidence,
  terminal/cutoff instants, elapsed, `Wall`, `Corrections`, `Reviews`,
  `Verifications`, tokens/cost as `Measured` with the summed `EventRef`s).
...
- `SourceSpec{ID, Kind, Repository, Roots, Ref}`; `SourceState`
  ...
  AllowEmpty}`; `SourceManifest`; `ReadBounds` with defaults
  (commits 5000, line 16 MiB, blob 16 MiB, total 512 MiB, processes 2).
...
| `ErrBoundExceeded` | Commit/byte/line/process cap stopped a read. | F3 |
| `ErrSourceCancelled` | Context ended mid-read (also wraps the context error). | F3 |
| `ErrInvalidSelection` | Blank or duplicate held-out run ID. | F3/F6 |
...
| R7 order-dependent evidence tie | `merge` keeps `a.TerminalEvidence` on ties (panel Claude-7). | `preferEvidence` total order. | FC-1 |
...
`ProducerDispatcherV0_1_0`). Per task, seq order:
...
| F1-EV-PROVENANCE-KEPT | Any recovered reading | `ReadingRef{SourceID, Repository, Path, Revision}` and `EventRef{Journal, Seq, Line}` on every field. | `FieldEvidence` | FC-1 |
...
| F3-SRC-MALFORMED-PARTIAL | One unreadable YAML among valid ones | Valid retained, `Counts.Malformed=1`, `SourcePartial`. | `SourceCounts` | FC-SOURCES |
...
| `Observation` | Gains `Wall *WallBreakdown` and `Evidence ObservationEvidence`; `Validate` does not inspect them. | Freezes the amended row without changing baseline validation. `ObservationEvidence` is kept comparable because `dispatched_test.go` compares rows with `!=`. |
...
- `gitEnvironment`, `preferEvidence`, `sortExamined` and `eventRefLess` are
  implemented but not yet called by the baseline path.
